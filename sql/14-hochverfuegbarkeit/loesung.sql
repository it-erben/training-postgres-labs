...
SELECT count(*) AS auftraege,
       count(DISTINCT nutzlast) AS verschiedene_nutzlasten
FROM tickets.auftrag;
-- Ergebnis: auftraege 2, verschiedene_nutzlasten 1. Beabsichtigt war ein
...
-- Schlüssel.
SELECT a.nutzlast AS gespeichert,
       gesendet.nutzlast AS gesendet,
       a.nutzlast = gesendet.nutzlast AS gleiche_nutzlast
FROM tickets.auftrag AS a
...
-- -- RW: replikat f. RO: replikat t, andere server_adresse.
--
...
-- Ergebnis: zwei Zeilen, auftraege 2, beide mit agent_id 7.
