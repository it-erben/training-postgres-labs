...
lässt. `ticket_metadata_gin` aus den Übungen 4 und 6 bleibt davon
...
2. Finde mit `pg_constraint` und `pg_index` alle Fremdschlüssel im Schema
   ...
   [Übung 6](../06-moderne-sql/AUFGABE.md) genau diese Spalte für jeden
...
`i.indpred IS NULL` lässt Teilindizes aus. `ticket_open_idx` aus Übung 4
...
Abfragen wie in Übung 6 lesen ohne Index bei jedem Rekursionsschritt die
gesamte Tabelle.
